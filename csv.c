...
static int read_next_field(FILE *fp, char field_delim, char text_delim,
                           CSV_FIELD *field) {
...
static int append_row(CSV_BUFFER *buffer) {
  ...
  temp_field =
      realloc(buffer->field, (buffer->rows + 1) * sizeof(CSV_FIELD **));
  if (temp_field != NULL) {
...
int csv_load(CSV_BUFFER *buffer, char *file_name) {
  ...
  while (!end) {
    ...
    if (!first) {
      next = read_next_field(fp, buffer->field_delim, buffer->text_delim,
                             buffer->field[i][j - 1]);
    }
...
int csv_get_field(char *dest, size_t dest_len, CSV_BUFFER *src, size_t row,
                  size_t entry) {
  int i;
  if (dest_len == 0)
    return 3;
  if (row >= src->rows                       /*row does not exist*/
      || row < 0 || entry >= src->width[row] /*entry does not exist*/
      || entry < 0) {
...
int csv_copy_row(CSV_BUFFER *dest, int dest_row, CSV_BUFFER *source,
                 int source_row) {
...
int csv_get_height(CSV_BUFFER *buffer) { return buffer->rows; }
